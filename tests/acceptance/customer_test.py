...
def test_create_customer_success_as_admin(auth_tokens):
...
def test_create_customer_success_as_manager(auth_tokens):
...
def test_create_customer_success_as_cashier(auth_tokens):
...
def test_create_customer_conflict(auth_tokens):
    resp = client.post(BASE_URL + "/customers", json=CUSTOMER_SAMPLE_1, headers=auth_header(auth_tokens, "admin"))
    assert resp.status_code == 409

def test_create_customer_missing_fields(auth_tokens):
    bad = {"name": ""}
...
def test_attach_loyalty_card_to_customer_success(auth_tokens):
...
def test_attach_loyalty_card_to_customer_customer_not_found(auth_tokens):
    resp = client.patch(BASE_URL + "/customers/9999/attach-card/0000000001", headers=auth_header(auth_tokens, "admin"))
    assert resp.status_code == 404

def test_attach_loyalty_card_to_customer_card_not_found(auth_tokens):
    resp = client.patch(BASE_URL + "/customers/1/attach-card/0000009999", headers=auth_header(auth_tokens, "admin"))
    assert resp.status_code == 404

def test_attach_loyalty_card_to_customer_already_attached_to_this_customer(auth_tokens):
    resp = client.patch(BASE_URL + "/customers/1/attach-card/0000000001", headers=auth_header(auth_tokens, "admin"))
    assert resp.status_code == 409

def test_attach_loyalty_card_to_customer_already_attached_to_other_customer(auth_tokens):
    resp = client.patch(BASE_URL + "/customers/2/attach-card/0000000001", headers=auth_header(auth_tokens, "admin"))
    assert resp.status_code == 409

def test_attach_loyalty_card_to_customer_unauthenticated():
...
def test_update_loyalty_card_points_success(auth_tokens):
...
def test_update_loyalty_card_points_card_not_found(auth_tokens):
    resp = client.patch(BASE_URL + "/customers/cards/9999?points=30", headers=auth_header(auth_tokens, "admin"))
    assert resp.status_code == 404

def test_update_loyalty_card_points_unauthenticated():
...
def test_update_customer_name_but_not_card_success(auth_tokens):
...
def test_update_customer_card_but_not_name_success(auth_tokens):
...
def test_update_customer_deletion_card_success(auth_tokens):
...
def test_update_customer_not_found(auth_tokens):
    payload = CUSTOMER_SAMPLE_1.copy()
    resp = client.put(BASE_URL + "/customers/9999", json=payload, headers=auth_header(auth_tokens, "admin"))
    assert resp.status_code == 404

def test_update_customer_card_not_found(auth_tokens):
...
def test_update_customer_name_conflict(auth_tokens):
    payload = CUSTOMER_SAMPLE_2.copy()
    resp = client.put(BASE_URL + "/customers/1", json=payload, headers=auth_header(auth_tokens, "admin"))
    if resp.status_code != 404:
        assert resp.status_code == 409

def test_update_customer_unauthenticated():
...
def test_delete_customer_success(auth_tokens):
...
def test_delete_customer_not_found(auth_tokens):
